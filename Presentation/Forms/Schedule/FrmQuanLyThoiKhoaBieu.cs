    ...
    public partial class FrmQuanLyThoiKhoaBieu : Form
    {
        ...
        private void dgvSchedules_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                cboMonHoc.SelectedValue = row.Cells["SubjectId"].Value;

                txtGiaoVien.Text = row.Cells["TeacherName"].Value.ToString();
                txtPhongHoc.Text = row.Cells["Room"].Value.ToString();

                dtpLessonDate.Value = (row.Cells["LessonDate"].Value == DBNull.Value) ? DateTime.Now : Convert.ToDateTime(row.Cells["LessonDate"].Value);
                numStartPeriod.Value = (row.Cells["StartPeriod"].Value == DBNull.Value) ? 1 : Convert.ToDecimal(row.Cells["StartPeriod"].Value);
                numEndPeriod.Value = (row.Cells["EndPeriod"].Value == DBNull.Value) ? 1 : Convert.ToDecimal(row.Cells["EndPeriod"].Value);
            }
        }
