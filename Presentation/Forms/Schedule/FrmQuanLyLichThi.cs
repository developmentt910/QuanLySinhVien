    ...
    public partial class FrmQuanLyLichThi : Form
    {
        ...
        private void dgvLichThi_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                SetDetailInputState(false);
            }
        }
        private void SetDetailInputState(bool isEnabled)
        {
            cboSemester.Enabled = isEnabled;
            dtpExamDateTime.Enabled = isEnabled;
            numThoiGianLamBai.Enabled = isEnabled;
            cboHinhThucThi.Enabled = isEnabled;
            txtPhongThi.Enabled = isEnabled;
        }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Bạn có chắc chắn muốn xóa lịch thi này?", "Xác nhận xóa", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                try
                {
